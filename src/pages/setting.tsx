...
import { Card, Row, Col, Button, Checkbox, Space, Radio, Input, Slider } from 'antd';
    ...
    <Card bordered={false}>
      <SettingBox>
        <Row gutter={[8, 8]}>
          ...
          <Col span={24}>
            <RaiseButton size="small" type="primary" danger>立即登陆</RaiseButton>
          </Col>
        </Row>
      </SettingBox>
      <SettingBox>
        <SettingModule title="常规">
          ...
          <MarginBottom>
            <Radio.Group onChange={(e) => setState({ groovePlayList: e.target.value })}
                         value={state.groovePlayList}>
              <RadioItem value={1}>
                <Text className="main">双击播放单曲时，用当前单曲所在的歌曲列表替换播放列表</Text>
                <Text className="intro">（节目同理）</Text>
              </RadioItem>
              <RadioItem value={2}>
                <Text className="main">双击播放单曲时，仅把当前单曲添加到播放列表</Text>
                <Text className="intro">（节目同理）</Text>
              </RadioItem>
            </Radio.Group>
          </MarginBottom>
      ...
      <SettingBox>
        <SettingModule title="消息与隐私">
          ...
          <MarginBottom>
            <Radio.Group onChange={(e) => setState({ groovePrivateLetter: e.target.value })}
          ...
          <MarginBottom>
            <Checkbox checked={state.dynamicFri} onChange={(e) => setState({ dynamicFri: e.target.checked })}>
              ...
              <Text className="intro">（关闭后，朋友页将不会出现新的推荐动态）</Text>
            </Checkbox>
          </MarginBottom>
          <MarginBottom>
          ...
          <MarginBottom>
            <Radio.Group onChange={(e) => setState({ ranking: e.target.value })}
                         value={state.ranking}>
              <RadioItem value={1}>
        ...
        <SettingModule title="下载设置">
          ...
          <MarginBottom>
            <Space size={32}>
              ...
              <Radio.Group onChange={(e) => setState({ auditionToneQuality: e.target.value })}
          ...
          <MarginBottom>
            <Space size={32}>
              ...
              <Radio.Group onChange={(e) => setState({ downloadToneQuality: e.target.value })}
            ...
            <Row align="middle" gutter={[8, 8]}>
              ...
              <Col span={8}>
                <Row align="middle" gutter={[8, 0]}>
                  <Col span={20}>
                    <Slider min={1} max={20} value={state.cache}
                            tipFormatter={formatter}
                            onChange={(value: number) => setState({ cache: value })}/>
                  </Col>
                  <Col span={4}>
                    <Text color="#3570bf" bold={true}>{CacheText}</Text>
                  </Col>
                </Row>
              </Col>
              <Col>
          ...
          <MarginBottom>
            <Radio.Group onChange={(e) => setState({ formatName: e.target.value })}
          ...
          <MarginBottom>
            <Radio.Group onChange={(e) => setState({ fileCategory: e.target.value })}
      ...
      <SettingBox>
        <SettingModule title="歌词">
          ...
          <MarginBottom>
            <Radio.Group onChange={(e) => setState({ lyricType: e.target.value })}
        ...
        <SettingModule title="关于网易云音乐">
          ...
          <MarginBottom>
            <Space>
              ...
              <RaiseButton size="small" type="primary" danger icon={<AppleFilled/>}>iPhone版</RaiseButton>
              <RaiseButton size="small" type="primary" danger icon={<AppleFilled/>}>iPad版</RaiseButton>
              <RaiseButton size="small" type="primary" danger icon={<AndroidFilled/>}>Android版</RaiseButton>
              <RaiseButton size="small" type="primary" danger icon={<WindowsFilled/>}>WP版</RaiseButton>
            </Space>
          </MarginBottom>
          <MarginBottom>
            <Space>
              <Link href="https://music.163.com/" target="_blank">《网易云音乐官网》</Link>
              <Link href="http://music.163.com/static/guideline.html" target="_blank">《网易云音乐社区管理细则》</Link>
              <Link href="https://st.music.163.com/official-terms/service" target="_blank">《服务条款》</Link>
              <Link href="https://st.music.163.com/official-terms/privacy" target="_blank">《隐私政策》</Link>
              <Link href="https://st.music.163.com/official-terms/children" target="_blank">《儿童隐私政策》</Link>
            </Space>
          </MarginBottom>
        </SettingModule>
      </SettingBox>
